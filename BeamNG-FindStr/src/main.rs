...
fn main() {
    ...
    if args.len() < 2 {
        println!("Usage: find_str.exe SearchString extension1,extension2(Optional)");
        return;
    }
    ...
    if search_for.len() == 0 {
        println!("Search string is to short");
        return;
    }
    ...
    let beamng = hkcu
        .open_subkey("SOFTWARE\\BeamNG\\BeamNG.drive")
        .expect("Cannot find HKCU/Software/BeamNG/BeamNG.drive. Game not installed?");
    let game_path: String = beamng.get_value("rootpath").expect(
        "Cannot find rootpath within HKCU/Software/BeamNG/BeamNG.drive. Game not installed?",
    );
    let game_path = &game_path;

    // program start
    println!(
        "Only parsing files with these extensions - {}",
        extensions_str
    );
    ...
    for file in &game_files {
        ...
        if extensions_map.contains_key(&file_extension.as_str()) {
            // if typical file
    ...
    for file in game_files {
        ...
        if file_extension.to_lowercase() == "zip" {
            ...
            for idx in 0..archive.len() {
                ...
                if !extensions_map.contains_key(&complete_ext.as_str()) {
                    continue;
                }
                ...
                let displayed_path = file.to_str().unwrap()[game_path.len()..].to_string()
                    + "/"
                    + name.to_str().unwrap();
